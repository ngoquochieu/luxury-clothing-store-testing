...
class AdminController {
  ...
  async addUser(req, res, next) {
    const { fullname, userPhone, password, address, date_of_birth, role } =
      req.body;
  ...
  async updateUser(req, res, next) {
    ...
    if (req.hasOwnProperty("file")) {
      const fileName = req.file.path;
      const newFileName = split_file_img(fileName);
      isUpdate = await Users.findByIdAndUpdate(req.params.id, {
        ...req.body,
        img: newFileName,
      });
    } else {
  ...
  async editItem(req, res, next) {
    ...
    const item = await Items.findOne({
      "details.product_code": req.params.product_code,
    });

    if (item)
      res.render("dashboard/products/db_edit_product", {
        admin,
        layout: "manager",
  ...
  async updateItems(req, res, next) {
    ...
    const newFilesName = files_name.map((file) => {
      return split_file_img(file.path);
    });
